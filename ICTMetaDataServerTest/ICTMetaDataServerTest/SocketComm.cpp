...
CSocketComm::CSocketComm() :
    ...
    m_hComm(INVALID_HANDLE_VALUE), m_hThread(NULL), m_hMutex(NULL), m_pPacket(NULL)
{
...
//  int nType: type of socket to create (SOCK_STREAM, SOCK_DGRAM)
///////////////////////////////////////////////////////////////////////////////
bool CSocketComm::ConnectTo(LPCTSTR strDestination, LPCTSTR strServiceName, int nFamily, int nType, DWORD dwTimeout )
{
    // Socket is already opened
    if ( IsOpen() )
        return false;
...
*/
		...
		if(dwTimeout){
			...
			if (iResult != NO_ERROR)
			{	
				TRACE("ioctlsocket failed with error: %ld\n", iResult);
			}
			    
			if(connect(sock,sockAddr,sockAddr.Size())==false)
			{	
				return false;
			}	
...
{
	m_pPacket = (stDataPacket*)malloc(1024*1024);
    DWORD   dwBytes  = 0L;
    DWORD   dwTimeout = INFINITE;
    LPBYTE  lpData = (LPBYTE)m_pPacket;
	SockAddrIn address;
...
*/
    ...
    while( IsOpen() )
    {
		...
		//read header
		DWORD	dwTargetReadBytes	= 20;
		DWORD	dwTotalReadBytes	= 0;
		DWORD	dwReadSize			= dwTargetReadBytes; 
		//lpData	= (LPBYTE)m_pPacket;

		//while(IsOpen()){
		//	dwBytes = ReadComm((LPBYTE)lpData, dwReadSize, dwTimeout);
		//	dwTotalReadBytes += dwBytes;
		//	// Error? - need to signal error
		//	if (dwBytes == (DWORD)-1L)	{
		//		// Do not send event if we are closing
		//		if (IsOpen())
		//			OnEvent( EVT_CONDROP, &address ); // lost connection
		//		return;
		//	}else if(dwTotalReadBytes == dwTargetReadBytes){
		//		
		//		break;
		//	}else{
		//		dwReadSize	= dwReadSize - dwBytes;
		//		lpData  = lpData + dwBytes;
		//		Sleep(0);
		//	}
		//}

//		TRACE("Read msgLength  Idnet [0x%08X] Size : %d,  Socket [%d]\n", m_pPacket->iden, m_pPacket->msgLength, m_hComm);
		
		dwTargetReadBytes	= sizeof(ICT_META_DATA);
		dwTotalReadBytes	= 0;
		dwReadSize			= dwTargetReadBytes; 
		lpData	= m_pPacket->pData;
		...
		while(IsOpen()){
			...
			if (dwBytes == (DWORD)-1L)	{
				...
				if (IsOpen())
					OnEvent( EVT_CONDROP, &address ); // lost connection
				return;
			}else if(dwTargetReadBytes == dwTotalReadBytes){
				
				break;
			}else{
				dwReadSize	= dwReadSize - dwBytes;
				lpData  = lpData + dwBytes;
				//printf("=== Read Byte[%d] Want Byte [%d] Remaid [%d] ===\n", dwBytes, Header.size, dwSize);
				Sleep(0);
			}
		}
		OnDataReceived( (LPBYTE)lpData, sizeof(ICT_META_DATA));
    }
}
...
bool connect2(char *host,int port, int timeout)
{
    TIMEVAL Timeout;
